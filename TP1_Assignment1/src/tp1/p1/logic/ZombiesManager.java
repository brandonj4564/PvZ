...
public class ZombiesManager {
	...
	/**
	 * Generate a random row for adding a new zombie and add the zombie to the current game
	 * 
	...
	public boolean addZombie(int row) {
		// Newly-added zombies will be put in the queue or in column Game.NUM_COLS before advancing into the board
	...
	// This one seems to be redundant as it has the same purpose as checkForEmptyTileAt()
	// The reason we switch from isPositionEmpty() to checkForEmptyTileAt() is because it is more natural to say row first and column second
	// Remember that isPositionEmpty(int col, int row) is what's expected in the provided code
	/**
	 * Check if tile at position <code> (col, row) </code> is not occupied by a zombie
	 * 
	...
	/**
	 * Check if a zombie is not occupying the tile at position <code>(row, col)</code> 
	 * 
	...
	/**
	 * Check if Zombie has lost the current Game or the player has won the current Game
	 * 
